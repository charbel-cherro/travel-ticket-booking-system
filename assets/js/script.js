document.addEventListener('DOMContentLoaded', function () {
  ...
  function getSelectedClassSeatCounts() {
    const selected = getSelectedFlightInput();

    return {
      first: selected ? Math.max(0, parseInt(selected.dataset.firstSeats || '2', 10)) : 2,
      business: selected ? Math.max(0, parseInt(selected.dataset.businessSeats || '3', 10)) : 3,
      economy: selected ? Math.max(0, parseInt(selected.dataset.economySeats || '10', 10)) : 10
    };
  }

  function generateSeatCodesForClass(classValue) {
    const counts = getSelectedClassSeatCounts();
    const totalForClass = counts[classValue] || 0;
    const seats = [];

    let startIndex = 0;
    if (classValue === 'business') {
      startIndex = counts.first;
    }
    if (classValue === 'economy') {
      startIndex = counts.first + counts.business;
    }

    for (let i = 0; i < totalForClass; i += 1) {
      const absoluteIndex = startIndex + i;
      const row = Math.floor(absoluteIndex / seatLetters.length) + 1;
      const letter = seatLetters[absoluteIndex % seatLetters.length];
  ...
  function getCurrentClassSeatCount() {
    const counts = getSelectedClassSeatCounts();
    return counts[classType.value] || 0;
  }

  const bookedSeatsByFlight = window.bookedSeatsByFlight || {};

  function bookedSeatKey(flightId, classValue, legType) {
    return `${parseInt(flightId || '0', 10)}|${String(classValue || 'economy').toLowerCase()}|${String(legType || 'outbound').toLowerCase()}`;
  }

  function getTakenSeatsForCurrentFlight(classValue, legType) {
    const selectedFlight = getSelectedFlightInput();
    const flightId = selectedFlight ? selectedFlight.value : 0;
    const key = bookedSeatKey(flightId, classValue, legType);
    return bookedSeatsByFlight[key] || [];
  }
  ...
  const seats = generateSeatCodesForClass(selectedClass);
  const unavailable = getTakenSeatsForCurrentFlight(selectedClass, prefix);

  let selectedSeats = [];

  if (!seats.length) {
    const message = document.createElement('p');
    message.className = 'muted small-inline';
    message.textContent = `No ${classLabel(selectedClass)} seats are available for this flight.`;
  ...
  seats.forEach(code => {
    ...
    if (unavailable.map(s => String(s).toUpperCase()).includes(code)) {
  ...
  function updateSummary() {
    ...
    seatZoneNote.textContent = `${classLabel(classType.value)} seats are currently shown (${getCurrentClassSeatCount()} seat(s) in this class).`;
  }
