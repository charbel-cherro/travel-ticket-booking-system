...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$flight && $tripType === 'multi') {
        $multiFromValues = array_values(array_filter(array_map('trim', $_POST['multiFrom'] ?? [])));
        $multiToValues = array_values(array_filter(array_map('trim', $_POST['multiTo'] ?? [])));
        $firstFrom = $multiFromValues[0] ?? 'Multi-city';
        $lastTo = $multiToValues ? end($multiToValues) : 'Custom route';
        $flight = [
            'id' => null,
            ...
            'from' => $firstFrom,
            'to' => $lastTo,
            'date' => $_POST['multiDate'][0] ?? date('Y-m-d'),
            'departure' => '--:--',
            'arrival' => '--:--',
            'stops' => max(0, count($multiToValues) - 1),
            'economy_price' => 0,
            'business_price' => 0,
            'first_price' => 0,
            'status' => 'active'
    ...
    $multiSegments = [];
    $multiFrom = $_POST['multiFrom'] ?? [];
    $multiTo = $_POST['multiTo'] ?? [];
    $multiDate = $_POST['multiDate'] ?? [];
    foreach ($multiFrom as $index => $fromCity) {
        $multiSegments[] = [
            'from' => trim($fromCity),
            'to' => trim($multiTo[$index] ?? ''),
            'date' => trim($multiDate[$index] ?? '')
        ];
    }
